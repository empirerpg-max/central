...
// === REAL TIME (3 COLUNAS RESTAURADAS) ===
async function loadRealTime() {
    ...
    const row = (l, c) => l.map(i => `
        <div class="chart-row" style="grid-template-columns: 30px 45px 1fr 80px;">
            ...
            <img src="${i.capa}">
            <div style="padding-left:10px;"><b style="font-size:12px;">${i.titulo}</b></div>
...
async function loadHOFList() {
    ...
    let h = `<h2 style="text-align:center; font-family:'Plus Jakarta Sans'; letter-spacing:6px; margin-bottom:40px;">DIRECTORY</h2><div id="hof-container">`;
    list.forEach(a => {
        ...
        h += `<div class="hof-card" onclick="loadHOFProfile('${a.name}')">
            ...
            <h3 style="font-size:13px; margin:0;">${a.name}</h3>
...
// === CHARTS (VARIÁVEIS FIXAS DO DOCX) ===
...
async function updateChart(el, tab, date) {
    ...
    area.innerHTML = res.map(i => {
        ...
        // USANDO i.semana e i.posicao conforme o seu banco de dados
...
// Chame buildMenu() no final do arquivo
buildMenu();
